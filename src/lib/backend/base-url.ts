 ...
 *
 * IMPORTANT:
 * - Usa bracket access process.env["X"] para evitar substituição estática no build.
 */
export function getBackendBaseUrl(): string {
  ...
  const raw =
    env["NEXT_PUBLIC_API_BASE_URL"] || // contrato “canônico” em PRD
    env["NEXT_PUBLIC_DECODER_BACKEND_BASE_URL"] ||
    env["DECODER_BACKEND_BASE_URL"] ||
    env["NEXT_PUBLIC_BACKEND_URL"] ||
    env["BACKEND_URL_PRD"] ||
    env["NEXT_PUBLIC_API_BASE_URL_PRD"] ||
    env["BACKEND_URL"] ||
    env["BACKEND_URL_LOCAL"] ||
    env["NEXT_PUBLIC_API_BASE_URL_LOCAL"] ||
    "http://127.0.0.1:4100";
