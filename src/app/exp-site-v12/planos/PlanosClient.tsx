...
export default function PlanosClient({ isAuthed }: { isAuthed: boolean }) {
  ...
  function onSubscribe(planId: string) {
    ...
    const checkoutPath = goToCheckout(planId);
    ...
    router.push(`/app/login?${qs.toString()}`);
  }
  ...
  return (
    <main className="flex-1 px-4 py-6 md:py-8">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        <div>
          <div className="text-xs tracking-[0.22em] text-zinc-400">PLANOS</div>
          <h1 className="mt-2 text-2xl md:text-3xl font-semibold">
            Escolha um plano e siga para o app.
          </h1>
          <p className="mt-2 text-sm text-zinc-400">
            Ao clicar em <span className="text-zinc-200">Assinar</span>, você
            faz login (ou cria sua conta) e segue para o checkout.
          </p>
        </div>
